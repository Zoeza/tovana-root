...
def report_manager(request, action):
    ...
    if action == 'create_nutrition_report':
        ...
        if request.method == 'POST':
            ...
            lactose_intolerance = report_actions.get_lactose_intolerance(data.get('lactose_intolerance_prs'))
            vitamin_B2 = report_actions.get_vitamin_B2(data.get('vitamin_b2_prs'))
            vitamin_B12 = report_actions.get_vitamin_B12(data.get('vitamin_b12_prs'))
            vitamin_C = report_actions.get_vitamin_C(data.get('vitamin_c_prs'))
            exercise_behavior = report_actions.get_exercise_behavior(data.get('exercise_behavior_prs'))
            power_and_strength = report_actions.get_power_and_strength(data.get('power_and_strength_prs'))
            endurance_training = report_actions.get_endurance_training(data.get('endurance_training_prs'))
            pain_sensitivity = report_actions.get_pain_sensitivity(data.get('pain_sensitivity_prs'))
            achilles_tendon_injury = report_actions.get_achilles_tendon_injury(data.get('achilles_tendon_injury_prs'))
            muscle_fatigue_and_cramping = report_actions.get_muscle_fatigue_and_cramping(
                data.get('muscle_fatigue_and_cramping_prs'))
            aerobic_capacity = report_actions.get_aerobic_capacity(data.get('aerobic_capacity_prs'))
            blood_pressure_response_to_exercise = report_actions.get_blood_pressure_response_to_exercise(
                data.get('exercise_behavior_prs'))
            bmi_response_to_exercise = report_actions.get_bmi_response_to_exercise(data.get('response_to_exercise_prs'))

            wet_vs_dry_earwax = report_actions.get_wet_vs_dry_earwax(data.get('wet_vs_dry_earwax_prs'))
            hair_loss_and_baldness = report_actions.get_hair_loss_and_baldness(data.get('hair_loss_and_baldness_prs'))
            dental_caries = report_actions.get_dental_caries(data.get('dental_caries_prs'))
            sleep_depth = report_actions.get_sleep_depth(data.get('response_to_exercise_prs'))
            warrior_vs_worrier = report_actions.get_warrior_vs_worrier(data.get('warrior_vs_worrier_prs'))
            ...
            context = {
                ...
                'power_and_strength_genotype_table': data.get('bitter_taste_perception_genotype_table'),
            ...
            return redirect('report-manager', 'convert-report')

        return redirect('report-manager', 'report-builder')
    if action == 'convert-report':
    ...
    if action == 'view_report':
        if request.method == 'POST':
            report_id = request.POST.get('report_id', False)
            selected_report = GeneratedReport.objects.all().get(id=report_id)
            url = "/tovana-root/src/templates/RJ85CBCAJP_Nutrition_Fitness_Wellness.docx"
